...
def pick_representative(race, model, target_stops: int, race_laps: int) -> int | None:
    """Among drivers who made exactly `target_stops` stops and ran the full
    race distance (excludes retirees who happen to share that stop count),
    the one with the fastest fitted baseline pace."""
    laps_run = race.laps.groupby("driver_number")["lap_number"].max()
    candidates = [
        d
        for d in model.driver_pace
        if num_stops(race, d) == target_stops and laps_run.get(d, 0) >= race_laps
    ]
    if not candidates:
        return None
    return min(candidates, key=lambda d: model.driver_pace[d])
...
def main() -> None:
    ...
    one_stop_driver = pick_representative(race, model, target_stops=1, race_laps=race_laps)
    two_stop_driver = pick_representative(race, model, target_stops=2, race_laps=race_laps)

    if one_stop_driver is None or two_stop_driver is None:
        print(
            "\nNo pair of full-distance 1-stop and 2-stop finishers to compare "
            "(one strategy may have had no full-distance finisher at all)."
        )
    else:
    ...
    print(f"\n=== Field-average car: best 1-stop vs best 2-stop ({high_deg} -> {low_deg}[, {low_deg}]) ===")
    avg_pace = sum(model.driver_pace.values()) / len(model.driver_pace)
    one_stop = best_one_stop(model, pit_loss, (high_deg, low_deg), avg_pace, race_laps)
    ...
    for first_stop in range(1, race_laps - 1):
        for second_stop in range(first_stop + 1, race_laps):
            plan = [(high_deg, first_stop), (low_deg, second_stop - first_stop), (low_deg, race_laps - second_stop)]
